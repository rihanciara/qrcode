				...
				@else
					<div class="@if(!empty($commission_agent)) col-sm-3 @else col-sm-4 @endif" style="display:none;">
						<div class="form-group">
							...
							{!! Form::select('status', ['final' => __('sale.final'), 'draft' => __('sale.draft'), 'quotation' => __('lang_v1.quotation')], 'final', ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'required']); !!}	</div>
					</div>
				@endif
				@if($sale_type != 'sales_order')
					<div class="col-sm-3" style="display:none;">
		        ...
		        @if(!empty($custom_field_1_label))
		        	...
		        	<div class="col-md-4">
				        <div class="form-group">
				            ...
				            {!! Form::text('custom_field_1', null, ['class' => 'form-control','placeholder' => $custom_field_1_label, 'required' => $is_custom_field_1_required,'readonly'=>true]); !!}
		        ...
		        @if(!empty($custom_field_2_label))
		        	...
		        	<div class="col-md-4">
				        <div class="form-group">
				            ...
				            {!! Form::text('custom_field_2', null, ['class' => 'form-control','placeholder' => $custom_field_2_label, 'required' => $is_custom_field_2_required,'readonly'=>true]); !!}
				        </div>
				    </div>
		        @endif
		      @if(!empty($custom_field_3_label))
		        	...
		        	<div class="col-md-4">
				        <div class="form-group">
				            ...
				            {!! Form::text('custom_field_3', null, ['class' => 'form-control','placeholder' => $custom_field_3_label, 'required' => $is_custom_field_3_required,'readonly'=>true]); !!}
				        </div>
				    </div>
		        @endif
...
<!-- Add the datetimepicker plugin script           format: 'DD-MM-YYYY hh:mm A',  -->
<script src="{{ asset('js/bootstrap-datetimepicker.min.js') }}"></script>

<script type="text/javascript">
    $(document).ready( function() {
        // Initialize datetimepicker for custom_field_1
        $('#custom_field_1').datetimepicker({
            format: moment_date_format + ' ' + moment_time_format,
            ignoreReadonly: true,
        });
        
        $('#custom_field_2').datetimepicker({
            format: moment_date_format + ' ' + moment_time_format,
            ignoreReadonly: true,
        });
        
       $('#custom_field_2').on('dp.change', function() {
    var checkin_date = $('#custom_field_1').val();
    var checkout_date = $('#custom_field_2').val();

    if (checkin_date && checkout_date) {
        var checkin_moment = moment(checkin_date, moment_date_format + ' ' + moment_time_format);
        var checkout_moment = moment(checkout_date, moment_date_format + ' ' + moment_time_format);
        
        // Set checkin time to 12PM
        checkin_moment.hour(12);
        checkin_moment.minute(0);
        checkin_moment.second(0);
        checkin_moment.millisecond(0);
        
        // Add one day if checkout time is after 12:00 PM
        if (checkout_moment.hour() >= 12 || (checkout_moment.hour() == 11 && checkout_moment.minute() >= 1)) {
            checkout_moment.add(1, 'day');
        }
        // Set checkout time to 12:00 PM
        checkout_moment.hour(12);
        checkout_moment.minute(0);
        checkout_moment.second(0);
        checkout_moment.millisecond(0);

        var stay_days = checkout_moment.diff(checkin_moment, 'days');

        // Set the total stayed days in custom_field_3
        $('#custom_field_3').val(stay_days);
    }
});
